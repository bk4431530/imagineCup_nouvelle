...
public class LevelMenu2D : Singleton<LevelMenu2D> {
	...
	public void createMenu ()
	{
		...
		if (showNextBackButtons == true)
		{
			if (nextButtonObject != null && backButtonObject != null)
			{
				...
				nextButtonObject.renderer.sortingOrder = itemsList.Count;
				backButtonObject.renderer.sortingOrder = itemsList.Count;
			}
		}
			...
			for (int i=1; i<itemsList.Count; i++)
			{
				...
				if (autoOffset)
				{
					if (orientation == MenuOrientation.Custom || orientation == MenuOrientation.Horizontal)
						itemOffset.x = itemsList[i-1].renderer.bounds.size.x + spacingBetweenItems;
					else if (orientation == MenuOrientation.Vertical)
						itemOffset.y = itemsList[i-1].renderer.bounds.size.y + spacingBetweenItems;
				}
				changePos.x += itemOffset.x;
				changePos.y += itemOffset.y;
				itemsList[i].transform.position = changePos;
				if (shouldIncreaseOrder)
					itemsList[i].renderer.sortingOrder = itemsList[i-1].renderer.sortingOrder + 1;
				else 
					itemsList[i].renderer.sortingOrder = itemsList[i-1].renderer.sortingOrder - 1;
			}
		}
	...
	public void sortOrderOfItems()
	{
		...
		{
			itemsList[i].renderer.sortingOrder = i-CurrentItem;
			//itemsList[i].GetComponent<ClickTouchScript>().enabled = true;
		}
		for (int i=itemsList.Count-1; i>CurrentItem; i--)
		{
			itemsList[i].renderer.sortingOrder = CurrentItem-i;
			//itemsList[i].GetComponent<ClickTouchScript>().enabled = true;
		}
		itemsList[CurrentItem].renderer.sortingOrder = 0;
	...
	{
		_isMoving = false;
		itemsList[CurrentItem].renderer.sortingOrder = itemsList.Count-1;
